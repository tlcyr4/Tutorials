...
def vehicle_setup():
    ...
    vehicle.mass_properties.operating_empty = vehicle.mass_properties.max_zero_fuel - vehicle.mass_properties.cargo
    vehicle.mass_properties.max_payload = vehicle.mass_properties.cargo
    vehicle.mass_properties.max_fuel = vehicle.mass_properties.cargo
...
def plot_mission(conditions, line_style='bo-'):
    ...
    for segment in conditions:
        time = segment.frames.inertial.time[:, 0] / Units.min
        Lift = -segment.frames.wind.lift_force_vector[:, 2]
        Drag = -segment.frames.wind.drag_force_vector[:, 0] / Units.lbf
        Thrust = segment.frames.body.thrust_force_vector[:, 0] / Units.lbf
        eta = segment.propulsion.throttle[:, 0]
        mdot = segment.weights.vehicle_mass_rate[:, 0]
        thrust = segment.frames.body.thrust_force_vector[:, 0]
    ...
    for segment in conditions:
        time = segment.frames.inertial.time[:, 0] / Units.min
        CLift = segment.aerodynamics.lift_coefficient[:, 0]
        CDrag = segment.aerodynamics.drag_coefficient[:, 0]
        Drag = -segment.frames.wind.drag_force_vector[:, 0]
        Thrust = segment.frames.body.thrust_force_vector[:, 0]
        aoa = segment.aerodynamics.angle_of_attack[:, 0] / Units.deg
        l_d = CLift / CDrag
    ...
    for segment in conditions:
        ...
        aoa = segment.aerodynamics.angle_of_attack[:, 0] / Units.deg
        l_d = CLift / CDrag
        mass = segment.weights.total_mass[:, 0] / Units.lb
        altitude = segment.freestream.altitude[:, 0] / Units.ft
        mdot = segment.weights.vehicle_mass_rate[:, 0]
        thrust = segment.frames.body.thrust_force_vector[:, 0]
        sfc = 3600. * mdot / 0.1019715 / thrust
    ...
    for segment in conditions:
        ...
        thrust = segment.frames.body.thrust_force_vector[:, 0]
        sfc = 3600. * mdot / 0.1019715 / thrust
        velocity = segment.freestream.velocity[:, 0]
        pressure = segment.freestream.pressure[:, 0]
        density = segment.freestream.density[:, 0]
        EAS = velocity * np.sqrt(density / 1.225)
        mach = segment.freestream.mach_number[:, 0]
